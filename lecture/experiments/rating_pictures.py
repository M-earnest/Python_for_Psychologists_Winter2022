...
start_message = visual.TextStim(win,
                                text="In this experiment you will rate different animals on a scale from 1 to 7 on their cuteness. Please press the spacebar to start.",
                                color='black', height=40)
...
for stimulus in list_img:
    
    ratingScale.reset()
    # simply pass the respective image by it's system path to ImageStim:
    img = os.path.join(img_folder, stimulus)
    image_stim = visual.ImageStim(win, image=img, pos=(0.0, 0.40), units="norm", size=1)

    while ratingScale.noResponse:  # show & update until a response has been made

        # display/draw task question to remind participants
        visual.TextStim(win, text='How cute do you rate the following?', pos=[0, -250], italic=True).draw()

...
snacks = [
    'Potato crisps', 'Pimientos de Padrón', 'Oreos', 'Nachos',
    'Mozarella Brötchen', 'everything sweet', 'Reeses', 'Chips',
    'Tortilla Chips - Western BBQ-Style',  'Duplo', 'gummy bears',
    'Reeses', 'Sushi', 'Kinder Bueno', 'bread chips'
]

animals = [
    'Cat', 'more Cat', 'Red Panda', 'another Cat',
    'Cats?', 'Chameleon', 'Orca', 'African Bush Elephant',
    'Giraffe', 'Eurasian eagle-owl'
]
...
blocks = {'artists': artists, 
          'snacks': snacks,
          'animals': animals
          }
# as python doesn't allow the shuffling of dictionaries we'll have to switch between lists and dicts
list_blocks = list(blocks.items())  # make list of dict
rnd.shuffle(list_blocks)  # shuffle list
blocks = dict(list_blocks)  # transform shuffled list back to dict
...
block_counter = 1  # define a counter to keep track of the block we're in

# Run through the trials, stimulus by stimulus
for block_key in blocks:
    # display/draw task/question to remind participants
    visual.TextStim(win, text=('Block number: ' + str(block_counter) + '/3'), pos=[0, -100], italic=True).draw()
    visual.TextStim(win, text=('Next you will be asked to rate: ' + block_key+ '\n Press Spacebar to continue.'), pos=[0, 60], height=40, italic=True).draw()
    win.flip()
    keys = event.waitKeys(keyList=['space', 'escape']) # wait for spacebar key press before advancing or quit when escape is pressed

    trial_counter = 0   # define a counter to keep track of the trial we're in
    
    #=====================
    # Start Trial
    #=====================

    for stimulus in blocks[block_key]:
        ...
        while ratingScale.noResponse:  # show & update until a response has been made
            ...
            visual.TextStim(win, text=stimulus, bold=True, pos=[0, 30], height=40).draw()
